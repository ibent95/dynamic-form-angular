...
export class PublicationFormVersionsConfigurationsManagementFormPublicationFormModalComponent implements OnInit {
  ...
  constructor() {
    ...
    this.formStatus = this.stateData?.form_status || AppFormStatus.CREATE;
  ...
  public onSelectedStepIndexChange(stepIndex: number): void {
    this.stepperSelectedIndex = stepIndex;
  }
  ...
  public onPublicationFormVersionChange(data: any): void {
    this.getMasterDataPublicationForms(null, data.uuid); // this.stateData.form.form_parent?.field_id
  ...
  public onFormCancelButtonClick(): void {
    ...
    dialogRef.afterClosed().subscribe((results: any) => {
      if (results.response) {
        //window.history.back();
        console.log('generalForm.getRawValue', this.generalForm.getRawValue(), this.generalForm.errors);
        this.dialogRef.close();
      }
    });
  }
